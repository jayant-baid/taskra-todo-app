...
export default function AdminUserDetailsPage({
  ...
  const taskDefinitions = useMemo<TaskDefinition[]>(
    () =>
  ...
  const analytics = useMemo(
    () => computeAnalytics(taskDefinitions, taskOccurrences, today),
    [taskDefinitions, taskOccurrences, today],
  );
  ...
  const weekSummaries = useMemo(
    () =>
      getWeekDays(weekCenter, true).map((date) =>
        computeDaySummary(date, taskDefinitions, taskOccurrences, today),
      ),
    [weekCenter, taskDefinitions, taskOccurrences, today],
  );
  const monthlyData = useMemo(
    () => computeMonthlyData(month, taskDefinitions, taskOccurrences, today),
    [month, taskDefinitions, taskOccurrences, today],
  );
      ...
      <main className="min-h-screen bg-[var(--bg-app)] p-6 text-[var(--text-primary)]">
        <div className="mx-auto max-w-xl rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-surface)] p-8 text-center">
          <ShieldCheck
            ...
            size={30}
          />
          <h1 className="text-lg font-semibold">Unable to open user details</h1>
    ...
    <div className="flex h-screen w-full flex-col overflow-hidden bg-[var(--bg-app)] text-[var(--text-primary)]">
      <header className="flex h-12 shrink-0 items-center justify-between border-b border-[var(--border-subtle)] bg-[var(--bg-surface)] px-4">
        ...
        <div className="flex items-center gap-2 text-xs text-[var(--accent-primary)]">
          <ShieldCheck size={14} /> Admin preview
        </div>
      </header>
      <main className="flex min-h-0 flex-1 flex-col overflow-hidden md:flex-row">
        <section className="flex min-h-0 w-full shrink-0 flex-1 flex-col border-b border-[var(--border-subtle)] p-4 md:w-[460px] md:border-b-0 md:border-r lg:w-[500px]">
          <TaskRail
            ...
            todayOccurrences={todayTasks}
            tomorrowOccurrences={tomorrowTasks}
            onToggle={() => undefined}
            onDelete={() => undefined}
            onEdit={() => undefined}
        ...
        <section className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-5">
          <CalendarDock
            ...
            onPrevWeek={() => setWeekCenter((value) => addDays(value, -7))}
            onNextWeek={() => setWeekCenter((value) => addDays(value, 7))}
            onCurrentWeek={() => setWeekCenter(today)}
            onPrevMonth={() => setMonth((value) => shiftMonth(value, -1))}
            onNextMonth={() => setMonth((value) => shiftMonth(value, 1))}
            onCurrentMonth={() => setMonth(today.slice(0, 7))}
          />
        </section>
      </main>
    </div>
...
function requireOccurrences(
  ...
  return computeDaySummary(date, definitions, occurrences, getLocalDateString())
    .occurrences;
}
